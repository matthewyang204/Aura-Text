<!-- PROJECT LOGO -->
<br />
<div align="center">
  <h1 align="center">Aura Text</h1>
  ![AURA TEXT](https://github.com/rohankishore/Aura-Text/assets/109947257/9b59cf71-e8f7-4244-be38-0ab647d8ded8)
  <p align="center">
    日本語訳
    <br />
    <a href="https://github.com/rohankishore/Aura-Text/wiki"><strong>ドキュメントを見る »</strong></a>
  </p>
</div>

## プロジェクトについて

Aura TextはPyQt6とPythonで構築された優れたテキスト/コードエディタです。

## 始め方

### 前提条件
- Windows 10 x64以降、Linuxカーネル6.x以降、またはmacOS Ventura以降
- Python 3.9以降
- `python -m venv venv`でvenvを作成し、`venv\Scripts\activate`で有効にすることを推奨
- `pip install -r requirements.txt`で依存関係をインストール

## 開発

## 貢献
貢献は大歓迎です。

## ライセンス
MITライセンスの下で配布されています。`LICENSE.txt`を参照してください。
