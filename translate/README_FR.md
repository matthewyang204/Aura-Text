<!-- PROJECT LOGO -->
<br />
<div align="center">
  <h1 align="center">Aura Text</h1>

  ![AURA TEXT](https://github.com/rohankishore/Aura-Text/assets/109947257/9b59cf71-e8f7-4244-be38-0ab647d8ded8)

  <p align="center">
    An IDE made entirely with Python
    <br />
    <a href="https://github.com/rohankishore/Aura-Text/wiki"><strong>Explore the docs »</strong></a>
    <br />
    <br />
    <a href="https://github.com/rohankishore/Aura-Text/issues">Report Bug</a>
    ·
    <a href="https://github.com/rohankishore/Aura-Text/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=">Request Feature</a>
  </p>
</div>
<br />
<div align="center">
  <h1 align="center">Aura Text</h1>
  ![AURA TEXT](https://github.com/rohankishore/Aura-Text/assets/109947257/9b59cf71-e8f7-4244-be38-0ab647d8ded8)
  <p align="center">
    Français
    <br />
    <a href="https://github.com/rohankishore/Aura-Text/wiki"><strong>Explorer la documentation »</strong></a>
  </p>
</div>

## À propos du projet

Aura Text est un excellent éditeur de texte/Code construit avec PyQt6 et Python.

## Premiers pas

### Prérequis
- Windows 10 x64 ou ultérieur, une distribution Linux avec noyau 6.x ou ultérieur, ou macOS Ventura ou ultérieur
- Python 3.9 ou ultérieur
- (Recommandé) `python -m venv venv` puis `venv\Scripts\activate`
- Installer les dépendances via `pip install -r requirements.txt`

## Développement

## Contribuer
Les contributions sont les bienvenues.

## Licence
Distribué sous licence MIT. Voir `LICENSE.txt`.
